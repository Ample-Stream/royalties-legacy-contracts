...
export const createContract = async ({ org, name, contractId }: IBody): Promise<Buffer> => {
	...
	return await htmlStringToBuffer(htmlDoc)

}

const htmlStringToBuffer = async (html: string): Promise<Buffer> => {
	return new Promise((resolve, _) => {
		const doc = new jsPDF()
		doc.html(html, {
			margin: 2,
			callback: (doc) => {
				resolve(Buffer.from(doc.output()))
			}
		})
	})
}
